...
// setupLogFileName genera nombre de archivo de log mejorado
...
func procedureHandler(w http.ResponseWriter, r *http.Request) {
	...
	if req.IsFunction {
		...
		retIndex := -1
		retType := "number" // Tipo por defecto
		for i, p := range req.Params {
			if strings.ToUpper(p.Direction) == "OUT" {
				retIndex = i
				if p.Type != "" {
					retType = strings.ToLower(p.Type)
				}
				break
			}
		}

		// Si no hay un OUT explícito, crear uno automáticamente
		if retIndex == -1 {
			log.Printf("[PROCEDURE] Función %s: creando valor de retorno automático (tipo: %s)", req.Name, retType)
		}
		...
		} else if retType == "number" || retType == "integer" || retType == "int" {
		...
		// Agregar el resto de parámetros (excepto el OUT de retorno si fue especificado)
		paramPos := 2
		for i, p := range req.Params {
			if i == retIndex {
				continue
			...
			if strings.ToUpper(p.Direction) == "OUT" {
				...
				// Parámetro IN: verificar si es fecha y parsearlo
	...
	for i, p := range req.Params {
		...
		if strings.ToUpper(p.Direction) == "OUT" {
			...
			// Detecci├│n autom├ítica de fechas por nombre o formato
	...
	go func() {
		...
		for _, p := range req.Params {
			if p.Direction == "OUT" {
				...
				// Detecci├│n autom├ítica de fechas
				if strings.Contains(strings.ToLower(p.Name), "fecha") || strings.Contains(strings.ToLower(p.Name), "periodo") {
					var t time.Time
					var err error
					if s, ok := p.Value.(string); ok {
